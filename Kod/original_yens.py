...
G = ox.graph_from_place("Chodel, Lubelskie, Poland", network_type="drive")
        ...
        for i in range(len(A[k-1])-2):
            ...
            for path in A:
                if root_path == path[:i+1]:
                    for key in G[path[i], path[i+1]]:
                        G.edges[path[i], path[i+1], key]["weight"] = float("inf")
                        G.nodes[path[i]]["visited"] = False
            ...
            for path in A:
                for i in range(len(path)-1):
                    if path[i] in G and path[i+1] in G[path[i]] and 0 in G[path[i]][path[i+1]]:
                        G.edges[path[i], path[i+1], 0]["weight"] = G.edges[path[i], path[i+1], 0]["length"] / G.edges[path[i], path[i+1], 0]["maxspeed"]
        if not B:
            break
        B.sort(key=lambda path: sum(G.edges[path[i], path[i+1], 0]["weight"] for i in range(len(path)-1) if path[i] in G and path[i+1] in G[path[i]] and 0 in G[path[i]][path[i+1]]))
        A.append(B[0])
        B = []
    return A
...
'''
K = 2
yen_paths = yen_algorithm(start, end, K)
